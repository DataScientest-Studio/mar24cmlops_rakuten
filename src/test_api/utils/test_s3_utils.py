...
from api.utils.s3_utils import (
    load_aws_cfg,
    create_s3_conn_from_creds,
    download_from_s3,
    upload_to_s3,
    upload_file_list_to_s3,
    upload_folder_to_s3,
)
...
def test_download_from_s3(setup):
    ...
    local_path = os.path.join(
        setup["local_test_folder"], "zazie.jpg"
    )  # Temporary local path for download
...
def test_upload_file_list_to_s3(setup):
    ...
    local_path_x_bucket_path_tuple = [
        (setup["testfile1_path"], "test/file1.txt"),
...
def test_upload_folder_to_s3(setup):  # A verifier pour le return
    ...
    uploaded_folder_name = upload_folder_to_s3(
        setup["cfg_path"], local_folder, s3_folder
    )
...
def test_check_db_folder_exists(setup):
    ...
    for file_name in expected_files:
        ...
        assert "Contents" in file_objects and any(
            obj["Key"] == file_path for obj in file_objects["Contents"]
        ), f"'{file_name}' not found in S3 bucket"
...
def test_check_model_repository(setup):
    ...
    production_objects = s3_conn.list_objects_v2(
        Bucket=bucket_name, Prefix=production_path
    )
    assert (
        "Contents" in production_objects
    ), f"'{production_path}' folder not found in S3 bucket"
    ...
    assert (
        "Contents" in staging_objects
    ), f"'{staging_path}' folder not found in S3 bucket"

    # Check if production/init_model/ folder exists
    init_model_path = production_path + "init_model/"
    init_model_objects = s3_conn.list_objects_v2(
        Bucket=bucket_name, Prefix=init_model_path
    )
    assert (
        "Contents" in init_model_objects
    ), f"'{init_model_path}' folder not found in S3 bucket"

    # Check if production/init_model/ folder is not empty
    assert (
        len(init_model_objects["Contents"]) > 0
    ), f"'{init_model_path}' folder is empty in S3 bucket"
