...
from api.utils.make_db import process_listing, init_user_table, create_table_from_pd_into_duckdb, download_initial_db, upload_db, init_db
    ...
    if 'Contents' in objects and objects['Contents']:
        ...
        if to_delete:
            ...
            if 'Errors' in delete_objects_response:
                ...
                print(f"Deleted {len(to_delete)} objects from S3 bucket '{bucket_name}'")
        else:
            print(f"No objects to delete in '{test_folder_prefix}' folder in S3 bucket '{bucket_name}'")
    else:
...
def test_process_listing():
    ...
    """
    listing_csv = """listing_id,price
    1,100
    2,200"""
    prdtypecode_csv = """listing_id,prdtypecode
    1,10
    2,20"""
    listing_path = 'data/tmp/listing.csv'
    prdtypecode_path = 'data/tmp/prdtypecode.csv'
    
    with open(listing_path, 'w') as f:
        f.write(listing_csv)
    with open(prdtypecode_path, 'w') as f:
        f.write(prdtypecode_csv)
    
    df = process_listing(listing_path, prdtypecode_path)
...
def test_init_user_table():
    ...
    assert bcrypt.verify('jc', df.loc[df['username'] == 'jc', 'hashed_password'].values[0])
...
def test_upload_db(setup):
    ...
    upload_db(setup['cfg_path'], local_path)
    s3_conn = create_s3_conn_from_creds(setup['cfg_path'])
    bucket_name = 'rakutenprojectbucket'
    current_datetime = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    s3_key = f'db/rakuten_{current_datetime}.duckdb'
