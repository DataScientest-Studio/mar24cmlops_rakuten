    ...
    try:
        ...
        for obj in response["Contents"]:
            ...
            local_file_path = os.path.join(
                local_download_path, os.path.relpath(s3_key, s3_prefix)
            )
...
def verify_and_download_models(cfg_path, model_list_file):
    ...
    for line in lines:
        ...
        if not os.path.exists(local_download_path):
            print(
                f"Model {model_name} version {version} not found locally. Downloading..."
            )
            download_model(
                cfg_path, model_name, version, local_download_path, is_production
            )
        else:
...
def list_model_repository_folders(cfg_path, is_production):
    ...
    try:
        ...
        model_folders = [
            prefix["Prefix"].split("/")[-2] for prefix in response["CommonPrefixes"]
        ]
        model_versions = {}
        ...
        for model in model_folders:
            ...
            response = s3_conn.list_objects_v2(
                Bucket=bucket_name, Prefix=model_prefix, Delimiter="/"
            )
